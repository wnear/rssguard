...
static bool streqi(const string &a, size_t offset_a, size_t len_a, const string &b, size_t offset_b,
                   size_t len_b)
{
...
string Part::get_body() const
{
    ...
    if (is_mime_type("text")) {
        ...
        if (!charset.empty() && !streqi(charset, "utf-8") && !streqi(charset, "us-ascii")
                && !streqi(charset, "ascii")) {
...
static string get_date_string(const chrono::system_clock::time_point &date =
...
void Part::generate_msgid(const string &domain)
{
    ...
    string msgid = "<" + base64_encode(string_view(reinterpret_cast<char *>(buf),
                                       sizeof buf)) + "@" + domain + ">";
