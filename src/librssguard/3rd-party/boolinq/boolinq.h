...
public:
    ...
    {
        return selectMany_i([apply](T value, int index) {
            return apply(value);
        });
    }
    ...
    Linq<std::tuple<Linq<S, T>, BytesDirection, T, int>, int> bytes(BytesDirection direction =
                BytesFirstToLast) const
    {
    ...
    template<typename TRet>
    Linq<std::tuple<Linq<S, T>, BytesDirection, int>, TRet> unbytes(BytesDirection direction =
                BytesFirstToLast) const
    {
    ...
    Linq<std::tuple<Linq<S, T>, BytesDirection, BitsDirection, T, int>, int> bits(
        BitsDirection bitsDir = BitsHighToLow, BytesDirection bytesDir = BytesFirstToLast) const
    {
    ...
    Linq<std::tuple<Linq<S, T>, BytesDirection, BitsDirection, int>, TRet> unbits(
        BitsDirection bitsDir = BitsHighToLow, BytesDirection bytesDir = BytesFirstToLast) const
    {
...
template<typename T>
Linq<std::pair<T, T>, typename std::iterator_traits<T>::value_type> from(const T &begin,
        const T &end)
{
