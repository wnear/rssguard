...
void Application::offerChanges() const
{
    if (isFirstRunCurrentVersion()) {
        qApp->showGuiMessage(QSL(APP_NAME),
...
{
    return sendMessage((QStringList() << APP_IS_RUNNING << Application::arguments().mid(1)).join(
                           ARGUMENTS_LIST_SEPARATOR));
}
...
{
    // Fallback folder.
    const QString home_folder = homeFolder() + QDir::separator() + QSL(APP_LOW_H_NAME) +
                                QDir::separator() + QSL("data");

    if (QDir().exists(home_folder)) {
        return home_folder;
    } else {
#if defined (Q_OS_ANDROID)
        return IOFactory::getSystemFolder(QStandardPaths::GenericDataLocation) + QDir::separator() + QSL(
                   APP_NAME);
#else
...
SystemTrayIcon *Application::trayIcon()
{
    if (m_trayIcon == nullptr) {
        ...
        connect(m_trayIcon, &SystemTrayIcon::shown, m_feedReader->feedsModel(),
                &FeedsModel::notifyWithCounts);
        connect(m_feedReader->feedsModel(), &FeedsModel::messageCountsChanged, m_trayIcon,
                &SystemTrayIcon::setNumber);
    }
...
void Application::onFeedUpdatesFinished(const FeedDownloadResults &results)
{
    if (!results.updatedFeeds().isEmpty()) {
        ...
        qApp->showGuiMessage(tr("New messages downloaded"), results.overview(10),
