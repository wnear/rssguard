...
SystemFactory::AutoStartStatus SystemFactory::autoStartStatus() const
{
    ...
    if (desktop_file_location.isEmpty()) {
        qWarningNN << LOGSEC_GUI <<
                   "Searching for auto-start function status failed. HOME variable not found.";
...
bool SystemFactory::setAutoStartStatus(AutoStartStatus new_status)
{
    ...
    switch (new_status) {
        case AutoStartStatus::Enabled: {
            ...
            const QString source_autostart_desktop_file = QString(APP_DESKTOP_ENTRY_PATH) + QDir::separator() +
                    APP_DESKTOP_SOURCE_ENTRY_FILE;
...
QList<UpdateInfo> SystemFactory::parseUpdatesFile(const QByteArray &updates_file) const
{
    ...
    for (QJsonValueRef i : document) {
        ...
        update.m_date = QDateTime::fromString(release["published_at"].toString(),
                                              QSL("yyyy-MM-ddTHH:mm:ssZ"));
