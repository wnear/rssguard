      ...
      m_autoUpdateTimer(new QTimer(this)), m_feedDownloader(nullptr)
{
    ...
    if (qApp->settings()->value(GROUP(Feeds), SETTING(Feeds::FeedsUpdateOnStartup)).toBool()) {
        ...
        QTimer::singleShot(qApp->settings()->value(GROUP(Feeds),
                           SETTING(Feeds::FeedsUpdateStartupDelay)).toDouble() * 1000,
                           this,
...
void FeedReader::updateAutoUpdateStatus()
{
    ...
    m_globalAutoUpdateInitialInterval = qApp->settings()->value(GROUP(Feeds),
                                        SETTING(Feeds::AutoUpdateInterval)).toInt();
    m_globalAutoUpdateRemainingInterval = m_globalAutoUpdateInitialInterval;
    m_globalAutoUpdateEnabled = qApp->settings()->value(GROUP(Feeds),
                                SETTING(Feeds::AutoUpdateEnabled)).toBool();
    m_globalAutoUpdateOnlyUnfocused = qApp->settings()->value(GROUP(Feeds),
...
void FeedReader::loadSavedMessageFilters()
{
    ...
    // all feeds.
    m_messageFilters = DatabaseQueries::getMessageFilters(qApp->database()->connection(
...
{
    auto *fltr = DatabaseQueries::addMessageFilter(qApp->database()->connection(
                     metaObject()->className()), title, script);
...
void FeedReader::removeMessageFilter(MessageFilter *filter)
{
    ...
    // Remove from DB.
    DatabaseQueries::removeMessageFilterAssignments(qApp->database()->connection(
                metaObject()->className()), filter->id());
    DatabaseQueries::removeMessageFilter(qApp->database()->connection(metaObject()->className()),
                                         filter->id());
...
{
    DatabaseQueries::updateMessageFilter(qApp->database()->connection(metaObject()->className()),
                                         filter);
}
...
{
    feed->removeMessageFilter(filter);
    DatabaseQueries::removeMessageFilterFromFeed(qApp->database()->connection(
                metaObject()->className()),
            feed->customId(),
            filter->id(),
            feed->getParentServiceRoot()->accountId());
...
void FeedReader::executeNextAutoUpdate()
{
    ...
    if (!qApp->feedUpdateLock()->tryLock()) {
        qDebugNN << LOGSEC_CORE <<
                 "Delaying scheduled feed auto-updates for one minute due to another running update.";
