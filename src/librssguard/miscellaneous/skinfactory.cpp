...
Skin SkinFactory::skinInfo(const QString &skin_name, bool *ok) const
{
    ...
    while (!base_skin_folders.isEmpty()) {
        const QString skin_folder = base_skin_folders.takeAt(0) + QDir::separator() + skin_name +
                                    QDir::separator();
        ...
        if (QFile::exists(metadata_file)) {
            ...
            if (!skin_file.open(QIODevice::Text | QIODevice::ReadOnly)
            ...
            QDomNodeList colors_of_palette = skin_node.namedItem(QSL("palette")).toElement().elementsByTagName(
                                                 QSL("color"));
            ...
            // be safely loaded.
            skin.m_layoutMarkupWrapper = QString::fromUtf8(IOFactory::readFile(skin_folder +
                                         QL1S("html_wrapper.html")));
            skin.m_layoutMarkupWrapper = skin.m_layoutMarkupWrapper.replace(QSL("##"),
                                         APP_SKIN_PATH + QL1S("/") + skin_name);
            skin.m_enclosureImageMarkup = QString::fromUtf8(IOFactory::readFile(skin_folder +
                                          QL1S("html_enclosure_image.html")));
            skin.m_enclosureImageMarkup = skin.m_enclosureImageMarkup.replace(QSL("##"),
                                          APP_SKIN_PATH + QL1S("/") + skin_name);
            skin.m_layoutMarkup = QString::fromUtf8(IOFactory::readFile(skin_folder +
                                                    QL1S("html_single_message.html")));
            skin.m_layoutMarkup = skin.m_layoutMarkup.replace(QSL("##"), APP_SKIN_PATH + QL1S("/") + skin_name);
            skin.m_enclosureMarkup = QString::fromUtf8(IOFactory::readFile(skin_folder +
                                     QL1S("html_enclosure_every.html")));
            skin.m_enclosureMarkup = skin.m_enclosureMarkup.replace(QSL("##"),
                                     APP_SKIN_PATH + QL1S("/") + skin_name);
            ...
            skin.m_adblocked = QString::fromUtf8(IOFactory::readFile(skin_folder +
                                                 QL1S("html_adblocked.html")));
