...
NewspaperPreviewer::NewspaperPreviewer(RootItem *root, QList<Message> messages, QWidget *parent)
    : TabContent(parent), m_ui(new Ui::NewspaperPreviewer), m_root(root),
      m_messages(std::move(messages))
{
    m_ui->setupUi(this);
    connect(m_ui->m_btnShowMoreMessages, &QPushButton::clicked, this,
            &NewspaperPreviewer::showMoreMessages);
    showMoreMessages();
}
...
void NewspaperPreviewer::showMoreMessages()
{
    if (!m_root.isNull()) {
        ...
        for (int i = 0; i < 10 && !m_messages.isEmpty(); i++) {
            ...
            connect(prev, &MessagePreviewer::markMessageImportant, this,
                    &NewspaperPreviewer::markMessageImportant);
        ...
        m_ui->m_btnShowMoreMessages->setText(tr("Show more messages (%n remaining)", "",
                                                m_messages.size()));
