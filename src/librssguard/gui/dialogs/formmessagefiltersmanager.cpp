...
FormMessageFiltersManager::FormMessageFiltersManager(FeedReader *reader,
        const QList<ServiceRoot *> &accounts, QWidget *parent)
      ...
      m_accounts(accounts), m_reader(reader), m_loadingFilter(false)
{
    ...
    connect(m_ui.m_txtTitle, &QLineEdit::textChanged, this,
            &FormMessageFiltersManager::saveSelectedFilter);
    connect(m_ui.m_txtScript, &QPlainTextEdit::textChanged, this,
            &FormMessageFiltersManager::saveSelectedFilter);
    connect(m_ui.m_btnTest, &QPushButton::clicked, this, &FormMessageFiltersManager::testFilter);
    connect(m_ui.m_btnBeautify, &QPushButton::clicked, this,
            &FormMessageFiltersManager::beautifyScript);
    ...
    connect(m_ui.m_btnUncheckAll, &QPushButton::clicked, m_feedsModel,
...
void FormMessageFiltersManager::saveSelectedFilter()
{
    ...
    if (fltr == nullptr || m_ui.m_txtTitle->text().isEmpty()
            || m_ui.m_txtScript->toPlainText().isEmpty()) {
        return;
    }
...
void FormMessageFiltersManager::testFilter()
{
    ...
    try {
        ...
        m_ui.m_txtErrors->setTextColor(decision == FilteringAction::Accept ? Qt::GlobalColor::darkGreen :
                                       Qt::GlobalColor::red);
        ...
        m_ui.m_txtErrors->setPlainText(tr("JavaScript-based filter contains errors: '%1'.").arg(
                                           ex.message()));
    }
...
void FormMessageFiltersManager::loadFilterFeedAssignments(MessageFilter *filter,
        ServiceRoot *account)
{
...
void FormMessageFiltersManager::initializeTestingMessage()
{
    ...
    m_ui.m_txtSampleContents->setPlainText(
        QSL("<p>Browsers usually insert quotation marks around the q element.</p>"
            "<p>WWF's goal is to: <q>Build a future where people live in harmony "
            "with nature.</q></p>"));
    m_ui.m_txtSampleCreatedOn->setText(QString::number(
