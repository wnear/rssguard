...
void FormSettings::applySettings()
{
    ...
    if (!panels_for_restart.isEmpty()) {
        const QStringList changed_settings_description = panels_for_restart.replaceInStrings(
...
void FormSettings::cancelSettings()
{
    ...
    if (changed_panels.isEmpty()) {
        ...
        const QStringList changed_settings_description = changed_panels.replaceInStrings(QRegularExpression(
                    QSL("^")),
                QString::fromUtf8(" • "));
