    ...
    : SettingsPanel(settings, parent), m_ui(new Ui::SettingsLocalization)
{
    ...
    connect(m_ui->m_treeLanguages, &QTreeWidget::currentItemChanged, this,
            &SettingsLocalization::requireRestart);
    connect(m_ui->m_treeLanguages, &QTreeWidget::currentItemChanged, this,
            &SettingsLocalization::dirtifySettings);
}
...
void SettingsLocalization::loadSettings()
{
    ...
    for (const Language &language : qApp->localization()->installedLanguages()) {
        ...
        item->setIcon(0, qApp->icons()->miscIcon(QString(FLAG_ICON_SUBFOLDER) + QDir::separator() +
                      language.m_code));
    }

    m_ui->m_treeLanguages->sortByColumn(0, Qt::AscendingOrder);
    QList<QTreeWidgetItem *> matching_items = m_ui->m_treeLanguages->findItems(
                qApp->localization()->loadedLanguage(), Qt::MatchContains, 1);
