...
StatusBar::StatusBar(QWidget *parent) : QStatusBar(parent), m_mutex(new Mutex(QMutex::NonRecursive,
            this))
{
    ...
    m_barProgressFeedsAction = new QAction(qApp->icons()->fromTheme(QSL("application-rss+xml")),
                                           tr("Feed update progress bar"), this);
    ...
    m_lblProgressFeedsAction = new QAction(qApp->icons()->fromTheme(QSL("application-rss+xml")),
                                           tr("Feed update label"), this);
    ...
    m_barProgressDownloadAction = new QAction(qApp->icons()->fromTheme(QSL("emblem-downloads")),
            tr("File download progress bar"), this);
    ...
    m_lblProgressDownloadAction = new QAction(qApp->icons()->fromTheme(QSL("emblem-downloads")),
            tr("File download label"), this);
    ...
    for (const QString &action_name : actions) {
        ...
        if (matching_action == m_barProgressDownloadAction) {
            ...
            if (action_name == SEPARATOR_ACTION_NAME) {
                ...
                connect(matching_action, &QAction::changed, tool_button,
                        &PlainToolButton::reactOnSenderActionChange);
            } else {
...
void StatusBar::loadSpecificActions(const QList<QAction *> &actions, bool initial_load)
{
    for (QAction *act : this->actions()) {
        QWidget *widget = act->property("widget").isValid() ? static_cast<QWidget *>
                          (act->property("widget").value<void *>()) : nullptr;
