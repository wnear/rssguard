...
WebViewer::WebViewer(QWidget *parent) : QWebEngineView(parent), m_root(nullptr)
{
    ...
    connect(page, &WebPage::messageStatusChangeRequested, this,
            &WebViewer::messageStatusChangeRequested);
    setPage(page);
    resetWebPageZoom();
}
...
void WebViewer::loadMessages(const QList<Message> &messages, RootItem *root)
{
    ...
    m_root = root;
    m_messageContents = skin.m_layoutMarkupWrapper.arg(messages.size() == 1 ? messages.at(
                            0).m_title : tr("Newspaper view"),
                        messages_layout);
...
void WebViewer::contextMenuEvent(QContextMenuEvent *event)
{
    ...
    if (menu_data.linkUrl().isValid()) {
        ...
        menu->addAction(qApp->icons()->fromTheme(QSL("")),
        tr("Open link in external browser"), [menu_data]() {
