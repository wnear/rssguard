...
CacheForServiceRoot::CacheForServiceRoot() : m_cacheSaveMutex(new Mutex(QMutex::NonRecursive,
            nullptr)) {}
...
void CacheForServiceRoot::addMessageStatesToCache(const QStringList &ids_of_messages,
...
void CacheForServiceRoot::saveCacheToFile(int acc_id)
{
    ...
    // Save to file.
    const QString file_cache = qApp->userDataFolder() + QDir::separator() + QString::number(
                                   acc_id) + "-cached-msgs.dat";
...
QPair<QMap<RootItem::ReadStatus, QStringList>, QMap<RootItem::Importance, QList<Message>>>
CacheForServiceRoot::takeMessageCache()
{
    m_cacheSaveMutex->lock();
    ...
    return QPair<QMap<RootItem::ReadStatus, QStringList>, QMap<RootItem::Importance, QList<Message>>>
           (cached_data_read, cached_data_imp);
}
