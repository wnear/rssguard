    ...
    if (input_feed == nullptr) {
        ...
        if (parent_to_select != nullptr) {
            if (parent_to_select->kind() == RootItem::Kind::Category) {
                m_ui->m_cmbParentCategory->setCurrentIndex(m_ui->m_cmbParentCategory->findData(QVariant::fromValue((
                            void *) parent_to_select)));
            } else if (parent_to_select->kind() == RootItem::Kind::Feed) {
                int target_item = m_ui->m_cmbParentCategory->findData(QVariant::fromValue((
                                      void *) parent_to_select->parent()));
...
void FormFeedDetails::onDescriptionChanged(const QString &new_description)
{
    if (new_description.simplified().isEmpty()) {
        m_ui->m_txtDescription->setStatus(LineEditWithStatus::StatusType::Warning,
...
{
    bool is_username_ok = !m_ui->m_gbAuthentication->isChecked()
...
{
    bool is_password_ok = !m_ui->m_gbAuthentication->isChecked()
...
{
    Feed::AutoUpdateType auto_update_type = static_cast<Feed::AutoUpdateType>
                                            (m_ui->m_cmbAutoUpdateType->itemData(new_index).toInt());
...
{
    QPair<StandardFeed *, QNetworkReply::NetworkError> result = StandardFeed::guessFeed(
                m_ui->m_txtUrl->lineEdit()->text(),
    ...
    if (result.first != nullptr) {
        ...
        m_ui->m_cmbType->setCurrentIndex(m_ui->m_cmbType->findData(QVariant::fromValue((
                                             int) result.first->type())));
...
void FormFeedDetails::createConnections()
{
    ...
    connect(m_ui->m_txtTitle->lineEdit(), &BaseLineEdit::textChanged, this,
            &FormFeedDetails::onTitleChanged);
    connect(m_ui->m_txtDescription->lineEdit(), &BaseLineEdit::textChanged, this,
            &FormFeedDetails::onDescriptionChanged);
    connect(m_ui->m_txtUrl->lineEdit(), &BaseLineEdit::textChanged, this,
            &FormFeedDetails::onUrlChanged);
    connect(m_ui->m_txtUsername->lineEdit(), &BaseLineEdit::textChanged, this,
            &FormFeedDetails::onUsernameChanged);
    connect(m_ui->m_txtPassword->lineEdit(), &BaseLineEdit::textChanged, this,
            &FormFeedDetails::onPasswordChanged);
    connect(m_ui->m_gbAuthentication, &QGroupBox::toggled, this,
            &FormFeedDetails::onAuthenticationSwitched);
...
void FormFeedDetails::setEditableFeed(Feed *editable_feed)
{
    ...
    m_ui->m_cmbAutoUpdateType->setCurrentIndex(m_ui->m_cmbAutoUpdateType->findData(QVariant::fromValue((
                int) editable_feed->autoUpdateType())));
...
void FormFeedDetails::initialize()
{
    ...
    std::sort(encoded_encodings.begin(), encoded_encodings.end(), [](const QString & lhs,
    const QString & rhs) {
