...
TtRssGetFeedsCategoriesResponse TtRssNetworkFactory::getFeedsCategories()
{
    ...
    if (result.isNotLoggedIn()) {
        ...
        login();
        json["sid"] = m_sessionId;
        network_reply = NetworkFactory::performNetworkOperation(m_fullUrl, timeout,
                        QJsonDocument(json).toJson(QJsonDocument::Compact),
                        result_raw,
                        QNetworkAccessManager::PostOperation,
                        headers);
...
RootItem *TtRssGetFeedsCategoriesResponse::feedsCategories(bool obtain_icons,
        QString base_address) const
{
    ...
    if (status() == TTRSS_API_STATUS_OK) {
        ...
        QJsonArray items_to_process =
            m_rawContent["content"].toObject()["categories"].toObject()["items"].toArray();
...
TtRssGetHeadlinesResponse::TtRssGetHeadlinesResponse(const QString &raw_content) : TtRssResponse(
        raw_content) {}
...
TtRssUpdateArticleResponse::TtRssUpdateArticleResponse(const QString &raw_content) : TtRssResponse(
        raw_content) {}
