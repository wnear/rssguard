...
{
    QPointer<FormTtRssFeedDetails> form_pointer = new FormTtRssFeedDetails(serviceRoot(),
            qApp->mainFormWidget());
...
{
    TtRssUnsubscribeFeedResponse response = serviceRoot()->network()->unsubscribeFeed(
            customId().toInt());
    ...
    if (response.code() == UFF_OK && removeItself()) {
        ...
        qWarning("TT-RSS: Unsubscribing from feed failed, received JSON: '%s'",
...
QList<Message> TtRssFeed::obtainNewMessages(bool *error_during_obtaining)
{
    ...
    do {
        TtRssGetHeadlinesResponse headlines = serviceRoot()->network()->getHeadlines(customId().toInt(),
                                              limit, skip,
                                              true, true, false,
