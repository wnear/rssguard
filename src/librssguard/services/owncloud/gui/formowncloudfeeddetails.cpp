...
void FormOwnCloudFeedDetails::apply()
{
    if (m_editableFeed != nullptr) {
        ...
        if (m_ui->m_txtTitle->lineEdit()->text() != m_editableFeed->title()) {
            if (!qobject_cast<OwnCloudServiceRoot *>(m_serviceRoot)->network()->renameFeed(
                        m_ui->m_txtTitle->lineEdit()->text(),
        ...
        new_feed_data->setAutoUpdateType(static_cast<Feed::AutoUpdateType>
                                         (m_ui->m_cmbAutoUpdateType->itemData(
        ...
        const int category_id = parent->kind() == RootItem::Kind::ServiceRoot ? 0 :
                                parent->customId().toInt();
        const bool response = qobject_cast<OwnCloudServiceRoot *>(m_serviceRoot)->network()->createFeed(
        ...
        if (response) {
            ...
            accept();
            qApp->showGuiMessage(tr("Feed added"), tr("Feed was added, triggering sync in now."),
