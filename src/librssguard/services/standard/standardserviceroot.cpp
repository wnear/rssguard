...
void StandardServiceRoot::start(bool freshly_activated)
{
    ...
    if (freshly_activated && getSubTree(RootItem::Kind::Feed).isEmpty()) {
        ...
        if (MessageBox::show(qApp->mainFormWidget(), QMessageBox::Question,
            ...
            try {
                ...
                MessageBox::show(qApp->mainFormWidget(), QMessageBox::Critical,
                                 tr("Error when loading initial feeds"), ex.message());
            }
        }
    }
...
void StandardServiceRoot::loadFromDatabase()
{
    ...
    Assignment feeds = DatabaseQueries::getFeeds<StandardFeed>(database,
...
QString StandardServiceRoot::processFeedUrl(const QString &feed_url)
{
    if (feed_url.startsWith(QL1S(URI_SCHEME_FEED_SHORT))) {
        ...
        if (without_feed_prefix.startsWith(QL1S("https:"))
                || without_feed_prefix.startsWith(QL1S("http:"))) {
...
bool StandardServiceRoot::mergeImportExportModel(FeedsImportExportModel *model,
        RootItem *target_root_node, QString &output_message)
{
...
{
    QScopedPointer<FormStandardImportExport> form(new FormStandardImportExport(this,
            qApp->mainFormWidget()));
...
QList<QAction *> StandardServiceRoot::serviceMenu()
{
    if (m_serviceMenu.isEmpty()) {
        ...
        auto *action_export_feeds = new QAction(qApp->icons()->fromTheme("document-export"),
                                                tr("Export feeds"), this);
        auto *action_import_feeds = new QAction(qApp->icons()->fromTheme("document-import"),
                                                tr("Import feeds"), this);
