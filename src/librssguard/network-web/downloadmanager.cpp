    ...
    m_gettingFileName(false), m_canceledFileSelect(false)
{
    ...
    m_requestFileName = qApp->settings()->value(GROUP(Downloads),
...
void DownloadItem::init()
{
    ...
    connect(m_reply, static_cast<void (QNetworkReply::*)(QNetworkReply::NetworkError)>
            (&QNetworkReply::error), this, &DownloadItem::error);
#endif
...
void DownloadItem::getFileName()
{
    ...
    const QString filename_for_prompt = qApp->settings()->value(GROUP(Downloads),
                                        SETTING(Downloads::TargetExplicitDirectory)).toString() +
                                        QDir::separator() +
    ...
    if (m_requestFileName) {
        ...
        chosen_filename = QFileDialog::getSaveFileName(this, tr("Select destination for downloaded file"),
                          filename_for_prompt);
...
void DownloadItem::tryAgain()
{
    ...
    QNetworkReply *new_download = qApp->downloadManager()->networkManager()->get(QNetworkRequest(
                                      m_url));
...
void DownloadItem::updateDownloadInfoLabel()
{
    ...
    if (running) {
        ...
        info = QString(tr("%1 of %2 (%3 per second) - %4")).arg(DownloadManager::dataString(
                    m_bytesReceived),
...
void DownloadItem::updateInfoAndUrlLabel()
{
    ...
    m_ui->m_lblLocalFilename->setText(tr("Local file: %1").arg(QDir::toNativeSeparators(
                                          m_output.fileName())));
}

DownloadManager::DownloadManager(QWidget *parent) : TabContent(parent),
    m_ui(new Ui::DownloadManager),
    m_autoSaver(new AutoSaver(this)), m_model(new DownloadModel(this)),
    m_networkManager(new SilentNetworkAccessManager(this)), m_iconProvider(nullptr),
    m_removePolicy(RemovePolicy::Never)
{
    ...
    setDownloadDirectory(qApp->settings()->value(GROUP(Downloads),
...
void DownloadManager::updateRow(DownloadItem *item)
{
    ...
    bool remove = item->downloadedSuccessfully()
                  && removePolicy() == RemovePolicy::OnSuccessfullDownload;
...
void DownloadManager::save() const
{
    ...
    for (int i = 0; i < m_downloads.count(); i++) {
        ...
        settings->setValue(GROUP(Downloads), QString(Downloads::ItemDone).arg(i),
    ...
    while (!(key = QString(Downloads::ItemUrl).arg(i)).isEmpty()
            && settings->contains(GROUP(Downloads), key)) {
...
void DownloadManager::load()
{
    ...
    // Restore the policy.
    m_removePolicy = static_cast<RemovePolicy>(settings->value(GROUP(Downloads),
                     SETTING(Downloads::RemovePolicy)).toInt());
    ...
    while (settings->contains(GROUP(Downloads), QString(Downloads::ItemUrl).arg(i))) {
        ...
        QString file_name = settings->value(GROUP(Downloads),
                                            QString(Downloads::ItemLocation).arg(i)).toString();
...
QMimeData *DownloadModel::mimeData(const QModelIndexList &indexes) const
{
    ...
    for (const QModelIndex &index : indexes) {
        ...
        urls.append(QUrl::fromLocalFile(QFileInfo(m_downloadManager->m_downloads.at(
                                            index.row())->m_output).absoluteFilePath()));
    }
