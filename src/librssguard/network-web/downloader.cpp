...
Downloader::Downloader(QObject *parent)
    : QObject(parent), m_activeReply(nullptr), m_downloadManager(new SilentNetworkAccessManager(this)),
      ...
      m_inputMultipartData(nullptr), m_targetProtected(false), m_targetUsername(QString()),
      m_targetPassword(QString()),
...
{
    manipulateData(url, QNetworkAccessManager::PostOperation, data, timeout, protected_contents,
                   username, password);
}
...
void Downloader::manipulateData(const QString &url, QNetworkAccessManager::Operation operation,
                                ...
                                bool protected_contents, const QString &username, const QString &password)
{
    manipulateData(url, operation, QByteArray(), multipart_data, timeout, protected_contents, username,
                   password);
}
                                ...
                                const QString &password)
{
    ...
    if (non_const_url.startsWith(URI_SCHEME_FEED)) {
        ...
        request.setUrl(non_const_url.replace(QRegularExpression(QString('^') + URI_SCHEME_FEED),
                                             QString(URI_SCHEME_HTTP)));
    } else {
...
void Downloader::finished()
{
    ...
    if (redirection_url.isValid()) {
        ...
        if (redirection_url.host().isEmpty()) {
            request.setUrl(QUrl(reply->request().url().scheme() + QSL("://") + reply->request().url().host() +
